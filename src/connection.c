...
static int read_to_ringbuffer(int fd, connection* conn)       //使用了readv但是好像并没有提高效率，不过使用了栈上数据，避免了malloc，free
{
    int nread2 = 65536;
...
static int read_to_ringbuffer2(int fd, connection* conn)         //另一种读数据方式
{
    int size = 0;
...
static void event_readable_callback(int fd, event* ev, void* arg)
{
    ...
    int nread = read_to_ringbuffer(fd, conn);
...
static void event_writable_callback(int fd, event* ev, void* arg)
{
    connection* conn = (connection*)arg;
    int size = 0;
    char* msg = ring_buffer_get_msg(conn->ring_buffer_write, &size);
    if (size > 0)  {
        ...
        event_disable_writing(conn->conn_event);
        debug_msg("there is no msg when writable callback!");
    }
}
